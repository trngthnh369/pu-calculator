            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {presets.map((preset) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.97 }}
                  className="bg-gray-800/70 hover:bg-gray-700 text-left p-4 rounded-xl border border-gray-700 transition-all duration-200 hover:border-cyan-500 hover:shadow-lg hover:shadow-cyan-500/20"
                >
                  <div className="font-mono text-sm text-cyan-400 mb-2 font-semibold">
                    {preset.name}
                  </div>
                  <div className="text-xs text-gray-500 space-y-1">
                    <div>T: {preset.thickness} mm</div>
                    <div>OD: {preset.outerDiameter} mm</div>
                    <div>L: {preset.length} mm</div>
                  </div>
                </motion.button>
              ))}
            </div>
